...
# ============================================================
# 数据集专用 Prompt 补丁（追加到通用 prompt 末尾）
# 针对不同数据集的平台特征做校准
# ============================================================
...
补充说明：当前数据来自微博（社交媒体平台，非正规新闻媒体）。
微博内容的天然特点：
- 信息通常是碎片化的，不会像正规新闻那样提供完整的来源、引用和背景
- "据网友称""听说""转发"等表述在微博上非常普遍，真实和虚假帖子都会使用
- 社交互动帖（旅行分享、日常讨论、评论转发）不包含新闻事实，不能因为"缺乏事实性内容"就判定虚假
- 突发事件的早期报道天然缺乏细节，这是时效性导致的，不代表虚假
- 不要选择 rhetorical_analysis 和 emotional_tone 维度，除非内容极端异常
- 优先选择 factual_verification、comparative_evidence、cross_modal_consistency""",

        "tool_prompt": """

平台校准（微博 - 社交媒体）：
当前内容来自微博，这是社交媒体平台而非正规新闻媒体。分析时必须调整你的判断标准：

1. "信息不完整"不等于虚假：微博帖子天然不会提供完整的来源引用、数据出处或详细背景。缺乏引用来源、描述笼统、没有具体数据来源，在微博上是正常的，不能作为判定虚假的依据。

2. "社交互动内容"不是新闻：如果内容是日常分享、旅行记录、评论讨论、情绪表达，不要因为"缺乏可验证的事实性陈述"就判为虚假——它本来就不是新闻报道。

3. 突发事件的早期报道天然不完整：如果是突发事件（爆炸、事故、灾难），早期微博往往只有片段信息、模糊细节，这是正常的实时报道特征。

4. 什么才是微博虚假新闻的真正特征：
   - 描述的具体事件经核实确认不存在或被篡改
   - 张冠李戴：把A事件的图片配到B事件上
   - 编造具体数字、人物、时间且与已知事实矛盾
   - 已被官方或权威媒体辟谣的内容

5. 判断原则：只有当你发现内容中的具体事实性声明与已知信息明确矛盾时，才判定为虚假。"无法验证" ≠ "虚假"。""",

        "mpre": """

微博平台校准提醒：
- 如果工具判定虚假的理由是"缺乏可验证来源""描述笼统""信息不完整"，请大幅降低这些判断的权重——微博内容天然如此
- 如果工具判定虚假的理由是"具体事实与已知信息矛盾""图文明显不符"，则应采信
- 社交互动帖（旅行、日常、评论）不应被判为虚假
- 重点关注：是否有明确的事实性矛盾，而非信息是否完整""",

        "cbdf": """

微博平台最终校准：
- 微博是社交媒体，不能用正规新闻的"完整性"和"来源可信度"标准来评判
- 如果工具判定虚假的核心理由是"缺乏来源/证据/细节"，这在微博上不构成虚假证据
- 只有当存在明确的事实性矛盾（如日期错误、人物张冠李戴、事件编造）时，才应判定虚假
- 如果 MPRE 和工具的虚假判定主要基于"信息不完整"而非"事实错误"，应倾向真实""",
    },
    "weibo21": {
        "mcp_planning": """

补充说明：当前数据来自微博（社交媒体平台）。
- 微博内容天然信息碎片化，不会提供完整来源引用
- "缺乏来源""描述笼统"在微博上是正常的，不代表虚假
- 优先选择 factual_verification、comparative_evidence、cross_modal_consistency""",

        "tool_prompt": """

平台校准（微博）：微博是社交媒体，不是正规新闻。
- "信息不完整""缺乏引用来源"在微博上是正常的，不能作为虚假依据
- 只有发现具体事实与已知信息明确矛盾时才判定虚假
- "无法验证" ≠ "虚假" """,
